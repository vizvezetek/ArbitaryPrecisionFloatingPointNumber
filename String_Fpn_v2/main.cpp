...
Fpn FPN_drop_wawe (Fpn x1, Fpn x2) { //  (0,0) glob_min: -1
	...
	Fpn a (  (part1/part2).getSign()+((part1/part2)).getNumber().substr(0,15) ,10);
...
{ 
    // example: +34.12(...)-26.54(...)
    return getRandInRangeWithPrec(1, 40) + getRandInRangeWithPrec(1, 40); 
}
...
{ 
	...
	// 		fitness++; 
	// // } 
    float f1 = stof( clearFpnStr( chromosome.substr(0,22) ) );
    float f2 = stof( clearFpnStr( chromosome.substr(22,22) ) );
    // cout << clearFpnStr( chromosome.substr(0,10))  <<"\t" << clearFpnStr( chromosome.substr(52,10) ) <<endl;
    // cout <<f1 <<"\t" << f2;

    // float ret = stof( FPN_drop_wawe( Fpn(clearFpnStr( chromosome.substr(0,10))) , Fpn(clearFpnStr( chromosome.substr(100,10))) ).getNumber() ) ;

	// Fpn x1(clearFpnStr( chromosome.substr(0,22 ) ) );
	// Fpn x2(clearFpnStr( chromosome.substr(22,22) ) );
    ...
    // float ret = f_sphere(f1,f2);

    float ret = stof(temp.getSign() + temp.getNumber().substr(0,20) );

    cout << "x1: " << x1<< "x2: " << x2<< "ret: " << ret << endl; 
	return ret;	 
}; 
...
{ 
	...
	{ 
		...
		cout<< "x1: "<< clearFpnStr( population[0].chromosome.substr(0,22)) <<" x2: "<< clearFpnStr( population[0].chromosome.substr(22,22) ) <<"\t"; 
